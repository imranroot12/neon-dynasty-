...
{#if hasAnticipation}
	<OnMount
		onmount={() => {
			context.eventEmitter.broadcast({ type: 'soundLoop', name: 'nd_sfx_anticipation' });
			context.eventEmitter.broadcast({
				type: 'soundFade',
				name: 'nd_sfx_anticipation',
				from: 0,
				to: 1,
				duration: SECOND,
			});
			...
			return () => {
				context.eventEmitter.broadcast({ type: 'soundStop', name: 'nd_sfx_anticipation' });
			};
		}}
	/>
{/if}
